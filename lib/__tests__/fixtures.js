// fixtures.js — 造临时 AGENTBOARD_HOME，隔离真实 tools/tips/apps
const fs = require('fs');
const os = require('os');
...
function makeAppsRegistry(apps) {
  return apps || [
    { id: 'a.example.com', name: '测试应用A', url: 'https://a.example.com', description: 'descA', host: 'test', status: 'live' },
    { id: 'b.example.com', name: '测试应用B', url: 'https://b.example.com', description: 'descB', host: 'test', status: 'live' }
  ];
}
...
function makeHome(opts) {
  ...
  makeAppsRegistry(opts.apps).forEach(function(a, i) {
    fs.writeFileSync(path.join(appsDir, a.id + '.json'), JSON.stringify(Object.assign({ order: i }, a), null, 2), 'utf8');
  });

  return { home: home, appsDir: appsDir, toolsDir: toolsDir, tipsDir: tipsDir };
}
