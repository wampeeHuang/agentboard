...
function treeSources() {
  return [
    ...
    { name: '使用说明书.html', path: path.join(PROJECT, 'docs', '使用说明书.html'), scope: ['lib', 'web', 'docs', 'principles', 'mechanisms', 'examples'] }
  ];
}
...
function auditTree() {
  ...
  return { ok: issues.length === 0, total: total, errors: issues.length, issues: issues, fixes: fixes };
}
