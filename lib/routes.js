...
var docsAudit = require('./docs-fresh');
...
// 工具架卡片拖拽排序真相源：state/ 文件（跨入口/跨浏览器共享），非浏览器 localStorage
var CARD_ORDER_FILE = path.join(process.env.AGENTBOARD_HOME || path.join(os.homedir(), '.agentboard'), 'state', 'tools-card-order.json');
function safeTipName(fname) {
  if (!fname || !/\.md$/.test(fname)) return null;
  // 根目录：仅文件名（防路径穿越）；独立口子目录：dsh/、cron/ 下的一层文件
  var parts = String(fname).split('/');
  if (parts.length === 1) {
    if (path.basename(fname) !== fname) return null;
    if (!/^[a-zA-Z0-9._-]+\.md$/.test(fname)) return null;
    return fname;
  }
  if (parts.length === 2) {
    if (parts[0] !== 'dsh' && parts[0] !== 'cron') return null;
    if (!/^[a-zA-Z0-9._-]+\.md$/.test(parts[1])) return null;
    return fname;
  }
  return null;
}
...
var SOURCES_MD = [
  ...
  '| 文件边界/架构 | `AGENTS.md` §架构 |',
  '| 技术选型 | `package.json`（以实际依赖为准） |',
  ''
].join('\n');

function registerRoutes(app, ctx) {
  var { read, esc, monogram, openFolder, safeResolve, renderMarkdown, scanTools, findManifest, startTool, stopTool, createTool, updateTool, getHealth, apiHTML, registry, TOOLS_DIR, SKILLS_DIR, TIPS_DIR, PRINCIPLES_DIR, LOCAL_SKILLS_DIR, PROJECT_DIR, apiLog, apiCounts } = ctx;
  ...
  app.get('/api/tools/schema', function(req, res) {
    res.json({
      ok: true,
      schema: {
        fields: schemaLoader.get().TOOL_FIELDS,
        formTypes: schemaLoader.get().FORM_TYPES,
        categoryValues: schemaLoader.get().CATEGORY_VALUES,
        categoryDefinitions: schemaLoader.get().CATEGORY_DEFINITIONS,
        catSuggest: schemaLoader.get().CAT_SUGGEST,
        runtimeValues: schemaLoader.get().FIELD_RULES.runtime.props.language.values
      }
    });
  });
  ...
  // 工具架拖拽排序持久化（GET 读 / POST 写 state/tools-card-order.json；body {order:[id,...]}）
  app.get('/api/tools/order', function(req, res) {
    try {
      var arr = JSON.parse(fs.readFileSync(CARD_ORDER_FILE, 'utf8'));
      res.json({ ok: true, order: Array.isArray(arr) ? arr : [] });
    } catch (e) {
      res.json({ ok: true, order: [] }); // 无文件/坏文件 = 空序 = 前端走默认分类序
    }
  });

  app.post('/api/tools/order', express.json(), function(req, res) {
    var order = req.body && req.body.order;
    if (!Array.isArray(order)) return res.status(400).json({ ok: false, error: 'order array required' });
    var clean = order.filter(function(id) { return typeof id === 'string' && /^[a-zA-Z0-9._-]{1,128}$/.test(id); });
    try {
      fs.mkdirSync(path.dirname(CARD_ORDER_FILE), { recursive: true });
      writeTextAtomic(CARD_ORDER_FILE, JSON.stringify(clean));
      res.json({ ok: true, count: clean.length });
    } catch (e) {
      res.status(500).json({ ok: false, error: 'write failed: ' + e.message });
    }
  });
  ...
  app.get('/api/tips', function(req, res) {
    try {
      ...
      if (domain) tips = tips.filter(function (t) { return (t.domain || 'general') === domain; });
      if (author) tips = tips.filter(function (t) { return (t.author || '其他') === author; });
  ...
  // 通配单条路由：支持根目录文件与独立口子目录（dsh/、cron/），兼容 %2F 编码与原始斜杠
  function tipFileParam(req) {
    var raw = req.params.file !== undefined ? req.params.file : req.params[0];
    try { return safeTipName(decodeURIComponent(String(raw || ''))); } catch (_) { return null; }
  }

  app.get('/api/tips/*', function(req, res) {
    var fname = tipFileParam(req);
    if (!fname) return res.status(400).json({ ok: false, error: 'invalid file name' });
  ...
  function tipBody(type, title, desc, source, tool, scenario, domain, author) {
  ...
  app.post('/api/tips', express.json(), function(req, res) {
    ...
    var domain = String(b.domain || 'general').trim();
    var author = String(b.author || '其他').trim();
    if (!title) return res.status(400).json({ ok: false, error: 'title required' });
    if (!type) return res.status(400).json({ ok: false, error: 'type required' });
    // 领域 → 独立口子目录（dsh/cron 进对应目录，其余落根目录 general）
    var domainDir = domain === 'dsh' ? 'dsh' : domain === 'cron' ? 'cron' : '';
    var targetDir = domainDir ? path.join(TIPS_DIR, domainDir) : TIPS_DIR;
    if (domainDir && !fs.existsSync(targetDir)) fs.mkdirSync(targetDir, { recursive: true });
    var now = new Date();
    var stamp = now.getFullYear() + p2(now.getMonth() + 1) + p2(now.getDate()) + '-' + p2(now.getHours()) + p2(now.getMinutes());
    var fname = 'tip-' + stamp + '.md';
    if (fs.existsSync(path.join(targetDir, fname))) fname = 'tip-' + stamp + p2(now.getSeconds()) + '.md';
    try { writeTextAtomic(path.join(targetDir, fname), tipBody(type, title, desc, source, tool, scenario, domain, author)); }
    catch (e) { return res.status(500).json({ ok: false, error: 'write failed: ' + e.message }); }
    res.json({ ok: true, file: domainDir ? domainDir + '/' + fname : fname });
  });

  app.put('/api/tips/*', express.json(), function(req, res) {
    var fname = tipFileParam(req);
    if (!fname) return res.status(400).json({ ok: false, error: 'invalid file name' });
    if (fname === 'CONSTITUTION.md' || fname === 'CHECKPOINT.md') return res.status(403).json({ ok: false, error: 'protected: ' + fname });
    var b = req.body || {};
    ...
    // domain/author 同款三态
    var domainBody = b.domain === undefined ? null : String(b.domain).trim();
    var authorBody = b.author === undefined ? null : String(b.author).trim();
    if (!title) return res.status(400).json({ ok: false, error: 'title required' });
    if (!type) return res.status(400).json({ ok: false, error: 'type required' });
    var file = path.join(TIPS_DIR, fname);
    if (!fs.existsSync(file)) return res.status(404).json({ ok: false, error: 'tip not found: ' + fname });
    ...
    var fm = existing.match(/^---\n([\s\S]*?)\n---\n?/);
    if (fm) fmLines = fm[1].split('\n');
    var hasType = false, hasDate = false, hasSource = false, hasTool = false, hasScenario = false, hasDomain = false, hasAuthor = false;
    var kept = fmLines.map(function(line) {
      ...
      if (/^domain:/.test(line)) { hasDomain = true; return domainBody === null ? line : (domainBody ? 'domain: ' + domainBody : null); }
      if (/^author:/.test(line)) { hasAuthor = true; return authorBody === null ? line : (authorBody ? 'author: ' + authorBody : null); }
      return line;
    ...
    if (domainBody && !hasDomain) kept.push('domain: ' + domainBody);
    if (authorBody && !hasAuthor) kept.push('author: ' + authorBody);
    var content = '---\n' + kept.join('\n') + '\n---\n\n# ' + title + '\n\n' + desc + '\n';
    try { writeTextAtomic(file, content); }
    catch (e) { return res.status(500).json({ ok: false, error: 'write failed: ' + e.message }); }
    res.json({ ok: true, file: fname });
  ...
  app.delete('/api/tips/*', function(req, res) {
    var fname = tipFileParam(req);
    if (!fname) return res.status(400).json({ ok: false, error: 'invalid file name' });
    if (fname === 'CONSTITUTION.md' || fname === 'CHECKPOINT.md') return res.status(403).json({ ok: false, error: 'protected: ' + fname });
    var file = path.join(TIPS_DIR, fname);
  ...
  app.get('/api/registry', function(req, res) {
    try {
      var governance = read(path.join(PROJECT_DIR, 'AGENTS.md'));
      var DOCS = [
        ['governance', '治理宪法', 'AGENTS.md', governance || ''],
  ...
  function parseTipFile(filePath) {
    ...
    return { title: title, desc: desc, body: md, type: tipType, source: fm.source || '', author: fm.author || '', date: fm.date || '', domain: fm.domain || '' };
  }

  // 全量列表：根目录（general）+ 独立口子目录（dsh/、cron/），file 带子目录前缀
  function listTips() {
    var out = [];
    var DOMAIN_DIRS = ['dsh', 'cron'];
    function scan(base, prefix) {
      if (!fs.existsSync(base)) return;
      var files = fs.readdirSync(base).filter(function (f) { return f.endsWith('.md') && f !== 'CONSTITUTION.md' && f !== 'CHECKPOINT.md'; }).sort();
      files.forEach(function (f) {
        var rel = prefix ? prefix + '/' + f : f;
        var tip = parseTipFile(path.join(base, f));
        if (tip) out.push({ file: rel, title: tip.title, desc: tip.desc, type: tip.type || 'reference', source: tip.source, author: tip.author, date: tip.date, domain: tip.domain || prefix || 'general' });
      });
    }
    scan(TIPS_DIR, '');
    DOMAIN_DIRS.forEach(function (d) { scan(path.join(TIPS_DIR, d), d); });
    return out;
  }

  // Tips (操作日志)
  if (fs.existsSync(TIPS_DIR)) {
    app.get('/tips', function(req, res) { res.redirect('/#tips'); });

    app.get('/tips/*', function(req, res) {
      var name = String(req.params[0] || '');
      try { name = decodeURIComponent(name); } catch (_) { name = ''; }
      var filePath = safeResolve(TIPS_DIR, name);
      if (!filePath) return res.status(403).send('forbidden');
  ...
  app.get('/principles', function(req, res) { res.redirect('/#principles'); });
  ...
  app.get('/api/audit', async function(req, res) {
    var schemaRes = schemaLoader.get().auditAll();
    var runtimeRes;
    try { runtimeRes = schemaLoader.get().auditRuntime(); } catch (e) { runtimeRes = { issues: [{ id: 'audit-runtime', name: '运行时漂移检查', errors: ['运行时检查失败: ' + e.message], warnings: [] }] }; }
    var brandRes, treeRes, docsRes;
    ...
    try { docsRes = docsAudit.auditDocs(); } catch (e) { docsRes = { ok: false, error: e.message }; }
