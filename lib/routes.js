...
// 从 lib/manifest-schema.js（真源）生成 Schema 文档，供系统规范页渲染
...
// 系统规范页"真相源索引"：每条治理主题只有一个家，这里是索引不是内容
var SOURCES_MD = [
  '# 治理真相源索引',
  '',
  '每条治理主题只有一个家，改了自动生效。本页是索引，不是内容。',
  '',
  '| 主题 | 唯一真相源 | 替代的旧文档 |',
  '|------|-----------|------------|',
  '| 治理宪法（原则/永不/骨件边界） | `agent.md`（本页"治理宪法"标签） | design-spec.md §一/§五/§六（已归档） |',
  '| 设计语言（配色/字体/组件/场景） | `skills/agentboard-design-system/`（SKILL.md + brand-dna.md + references/） | design-spec.md §四（已归档） |',
  '| Manifest Schema（字段/枚举/校验） | `lib/manifest-schema.js`（本页"Manifest Schema"标签） | design-spec.md §三（已归档） |',
  '| 落盘规则 | `~/.claude/CLAUDE.md` §文件纪律、§落盘位置约定 | repo-spec.md §二（已归档） |',
  '| 操作日志格式/分类 | `tips/CONSTITUTION.md` | repo-spec.md §四（已归档） |',
  '| 备份与恢复 | `F:\\warehouse\\inbox\\CLAUDE.md` | repo-spec.md §六（已归档） |',
  '| 文件边界/架构 | `agent.md` §架构 | repo-spec.md §一（已归档） |',
  '| 技术选型 | `package.json`（以实际依赖为准） | repo-spec.md §三（已归档） |',
  ''
].join('\n');
...
function registerRoutes(app, ctx) {
  ...
  // 系统规范页：治理宪法 + Manifest Schema + 真相源索引（三篇都从真源生成，不维护第二份拷贝）
