...
function registerRoutes(app, ctx) {
  ...
  app.get('/api/apps', function(req, res) {
    try {
      var ar = JSON.parse(fs.readFileSync(path.join(PROJECT_DIR, 'apps-registry.json'), 'utf8'));
  ...
  app.post('/api/apps', express.json(), function(req, res) {
    ...
    var file = path.join(PROJECT_DIR, 'apps-registry.json');
    var ar;
  ...
  // 系统规范页：设计规范 + 工程规范两篇文档（docs/*.md 原样返回 markdown，前端 md2html 渲染）
  app.get('/api/registry', function(req, res) {
    try {
      var DOCS = [['design', '设计规范', 'design-spec.md'], ['repo', '工程规范', 'repo-spec.md']];
      var docs = DOCS.map(function(d) {
        var p = path.join(PROJECT_DIR, 'docs', d[2]);
        if (!fs.existsSync(p)) return { key: d[0], title: d[1], file: d[2], markdown: '' };
        return { key: d[0], title: d[1], file: d[2], markdown: read(p) };
      });
