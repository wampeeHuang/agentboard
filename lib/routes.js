...
function registerRoutes(app, ctx) {
  var { read, esc, monogram, listDirs, openFolder, safeResolve, getChineseName, scanAllSkills, renderMarkdown, scanTools, findManifest, startTool, stopTool, createTool, updateTool, skillIndexHTML, commandsIndexHTML, getHealth, apiHTML, registry, TOOLS_DIR, SKILLS_DIR, TIPS_DIR, PRINCIPLES_DIR, LOCAL_SKILLS_DIR, PROJECT_DIR, apiLog, apiCounts, BUILTIN_COMMANDS } = ctx;

  // 测试钩子：apps-registry.json 真相源路径可被 env 覆盖（默认仓库根）
  ...
  app.get('/api/apps', function(req, res) {
    try {
      var ar = JSON.parse(fs.readFileSync(APPS_REG, 'utf8'));
      res.json({ ok: true, apps: ar.apps || [] });
    } catch (e) {
      res.json({ ok: false, error: 'apps-registry.json unreadable: ' + e.message });
    }
  });

  // ── 应用写回（apps-registry.json 真相源） ──
  app.post('/api/apps', express.json(), function(req, res) {
    ...
    var file = APPS_REG;
    var ar;
    try { ar = JSON.parse(fs.readFileSync(file, 'utf8')); } catch (e) { return res.status(500).json({ ok: false, error: 'apps-registry.json unreadable: ' + e.message }); }
    ar.apps = ar.apps || [];
    if (ar.apps.some(function(a) { return a.id === domain; })) return res.status(409).json({ ok: false, error: 'app exists: ' + domain });
    ...
    var app = {
      ...
      category: b.category || '网站',
      status: 'live',
      host: host
    };
    ar.apps.push(app);
    ar.updated = new Date().toISOString().slice(0, 16);
    try { registry.writeManifestAtomic(file, ar); } catch (e) { return res.status(500).json({ ok: false, error: 'write failed: ' + e.message }); }
  ...
  app.put('/api/apps/:id', express.json(), function(req, res) {
    ...
    try { ar = JSON.parse(fs.readFileSync(file, 'utf8')); } catch (e) { return res.status(500).json({ ok: false, error: 'apps-registry.json unreadable: ' + e.message }); }
    ar.apps = ar.apps || [];
    var idx = ar.apps.findIndex(function(a) { return a.id === req.params.id; });
    if (idx < 0) return res.status(404).json({ ok: false, error: 'app not found: ' + req.params.id });
    var app = ar.apps[idx];
    ...
    if (b.domain !== undefined) {
      ...
      if (dom !== app.id && ar.apps.some(function(a) { return a.id === dom; })) return res.status(409).json({ ok: false, error: 'app exists: ' + dom });
      app.id = dom;
  ...
  app.delete('/api/apps/:id', function(req, res) {
    ...
    var idx = ar.apps.findIndex(function(a) { return a.id === req.params.id; });
    if (idx < 0) return res.status(404).json({ ok: false, error: 'app not found: ' + req.params.id });
    var removed = ar.apps.splice(idx, 1)[0];
    ar.updated = new Date().toISOString().slice(0, 16);
    try { registry.writeManifestAtomic(file, ar); } catch (e) { return res.status(500).json({ ok: false, error: 'write failed: ' + e.message }); }
  ...
  function tipBody(type, title, desc) {
  ...
  app.post('/api/tips', express.json(), function(req, res) {
    ...
    try { writeTextAtomic(path.join(TIPS_DIR, fname), tipBody(type, title, desc)); }
    catch (e) { return res.status(500).json({ ok: false, error: 'write failed: ' + e.message }); }
    res.json({ ok: true, file: fname });
  });
  ...
  app.get('/catalog', function(req, res) {
    ...
    var diagBody = '<style>.diagram-grid{display:grid;grid-template-columns:repeat(auto-fill, minmax(320px, 1fr));gap:12px;margin-top:8px}.diagram-card{background:var(--paper);padding:20px;display:flex;align-items:flex-start;gap:14px;transition:transform .15s,box-shadow .15s;box-shadow:var(--shadow-border),var(--shadow-card);text-decoration:none;color:inherit}.diagram-card:hover{transform:translateY(-1px);box-shadow:var(--shadow-border),var(--shadow-card-hover)}.diagram-card .card-mono{flex-shrink:0;width:44px;height:44px;background:var(--green);color:var(--on-brand);display:flex;align-items:center;justify-content:center;font-family:"Cascadia Code","Consolas","SF Mono",monospace;font-size:14px;font-weight:500}.diagram-card .card-info{flex:1;min-width:0}.diagram-card .card-name{font-size:16px;font-weight:300;letter-spacing:-0.01em;line-height:1.35}.diagram-card .card-sub{font-size:11px;font-family:"Cascadia Code","Consolas","SF Mono",monospace;color:var(--text-muted);margin-top:1px}.diagram-card .card-link{font-size:11px;color:var(--green);font-weight:500;margin-top:6px}</style><div class="diagram-grid">' + (diagCards || '<p>暂无架构图</p>') + '</div>';
