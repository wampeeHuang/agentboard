...
function registerRoutes(app, ctx) {
  ...
  // 打开全局宪法：资源管理器定位到 ~/.claude/CLAUDE.md
  app.get('/open-constitution', function(req, res) {
    var f = path.join(os.homedir(), '.claude', 'CLAUDE.md');
    if (!fs.existsSync(f)) return res.status(404).json({ ok: false, error: 'CLAUDE.md 不存在' });
    require('child_process').exec('explorer /select,"' + f + '"');
    res.json({ ok: true, path: f });
  });
  ...
  app.get('/api/catalog/data', function(req, res) {
    ...
    var sections = [];
    if (globalMd) globalMd.split('\n').forEach(function(line) {
      var m = line.match(/^##\s+(.+)/);
      if (m) sections.push(m[1].trim());
    });
    var globalHtml = globalMd ? renderMarkdown(globalMd) : '<p>CLAUDE.md 未找到</p>';
    var secN = 0;
    globalHtml = globalHtml.replace(/<h2>/g, function() { secN++; return '<h2 id="sec-' + secN + '">'; });
    var global = { path: globalMdPath, lines: globalMd ? globalMd.split('\n').length : 0, html: globalHtml, sections: sections };
