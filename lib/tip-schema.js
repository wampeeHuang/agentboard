...
// ── 领域 / 作者 枚举（独立口 + 来源，防漂移）──
var DOMAIN_VALUES = ['general', 'dsh', 'cron'];
var AUTHOR_VALUES = ['人工', 'dsh-agent', 'claude', 'codex', 'cron', '其他'];
...
function validateTip(md) {
  ...
  // domain/author：缺失用默认值（general/其他），非法值报错防漂移
  var domain = fields.domain || 'general';
  if (DOMAIN_VALUES.indexOf(domain) === -1) {
    errors.push('domain 值无效: "' + domain + '"，合法值: ' + DOMAIN_VALUES.join('|'));
  }
  var author = fields.author || '其他';
  if (AUTHOR_VALUES.indexOf(author) === -1) {
    errors.push('author 值无效: "' + author + '"，合法值: ' + AUTHOR_VALUES.join('|'));
  }
...
function auditTips(dir) {
  ...
  var subdirs = DOMAIN_DIRS.filter(function (d) { return fs.existsSync(path.join(dir, d)); });

  function scan(base, prefix) {
    var files = fs.readdirSync(base).filter(function (f) {
      return f.endsWith('.md') && SKIP.indexOf(f) === -1;
    }).sort();
    files.forEach(function (f) {
      total++;
      var md;
      try { md = fs.readFileSync(path.join(base, f), 'utf8'); } catch (_) { return; }
      var r = validateTip(md);
      if (r.errors.length > 0 || r.warnings.length > 0) {
        issues.push({ file: (prefix ? prefix + '/' : '') + f, type: r.fields.type || '', errors: r.errors, warnings: r.warnings });
  ...
  scan(dir, '');
  subdirs.forEach(function (d) { scan(path.join(dir, d), d); });
