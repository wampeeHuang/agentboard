...
// Used by: lib/mcp-http.js (Streamable HTTP), server.js (REST)
...
var TOOL_DEFS = [
  ...
  },
  {
    ...
    description: '创建新工具并写入 manifest.json。写入前自动校验 schema（必填字段、owner 枚举、capability 长度等），不通过则驳回并返回具体错误。必填字段: id, name, description(须含【用途】), capability(≤30字)。type 默认为 service，可选 service/cli/api/folder/group。category 必须是已存在的分类(先调 list_tools 看现有分类)。owner 可选(自建/外部)。',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        owner: { type: 'string', description: '所有者: 自建/外部 (可选)' },
        category: { type: 'string', description: '分类名。必须先调 list_tools 查看已有分类，勿自创' },
        type: { type: 'string', description: '卡片类型: service(默认), cli, api, folder, group。选错=无按钮' },
      ...
      required: ['id', 'name', 'description', 'capability']
    }
  },
  {
...
// 与 tool-registry.createTool 同源：id 只允许 a-z 开头 + a-z 0-9 - _，防 ../ 逃逸
var ID_RE = /^[a-z][a-z0-9_-]*$/;
function validToolId(id) { return !!id && ID_RE.test(id); }
...
function handleGetTool(args) {
  ...
  if (!validToolId(args.id)) return textResult('Error: invalid tool id (must start with a letter, contain only a-z 0-9 - _): ' + args.id, true);
