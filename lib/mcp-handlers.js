...
var TOOL_DEFS = [
  ...
  },
  {
    ...
    description: '创建新工具并写入 manifest.json。写入前自动校验 schema（必填字段、owner 枚举、capability 长度等），不通过则驳回并返回具体错误。必填字段: id, name, description(须含【用途】), capability(≤30字), owner(自建/外部/AI托管)。type 默认为 service，可选 service/cli/api/folder/group。category 必须是已存在的分类(先调 list_tools 看现有分类)。',
    inputSchema: {
      type: 'object',
      properties: {
