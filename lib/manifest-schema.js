...
var CATEGORY_VALUES = ['模型', 'Agent', '设施', '获取', '查阅', '创作', '职能', '工作区'];

// ── 分类定义 (frontmatter — 校验门禁第一步: AI 必须先读此块再赋 category) ──
// 每项含: desc(一句话定义), scent(命中该类的关键词), anti(容易误归进来的反例)
var CATEGORY_DEFINITIONS = {
  '模型': {
    desc: 'AI 模型服务端点、推理 API、模型 serving',
    scent: ['文生图', '文生视频', 'TTS', 'embedding', '模型服务', '推理端点', '视觉模型', '语音合成模型', '视频生成模型', 'Claude Opus', 'Claude Sonnet', 'DeepSeek-R', 'DeepSeek-V', 'token用量'],
    anti: '不是"用了模型的工具"，而是"模型本身的服务端点"。应用类工具即使调用 AI 也不归此类'
  },
  'Agent': {
