...
var CATEGORY_VALUES = ['模型', 'Agent', '设施', '获取', '查阅', '创作', '职能'];

// ── 分类定义 (frontmatter — 校验门禁第一步: AI 必须先读此块再赋 category) ──
// 每项含: desc(一句话定义), scent(命中该类的关键词), anti(容易误归进来的反例)
var CATEGORY_DEFINITIONS = {
  '模型': {
    desc: 'AI 模型能力：本地推理或云端 API 的调用入口（本地/远程由 loc 字段区分）',
    scent: ['模型', 'LLM', '推理', '视觉', '语音', '生图', '模型服务', '模型端点', '模型聚合', '网关'],
    anti: '本地/远程用 loc 字段区分，不拆进分类。模型管理/聚合/中继网关归此类'
  },
  'Agent': {
...
var FIELD_RULES = {
  ...
  categoryAck: { type: 'boolean', label: '分类已确认' },
  loc:         { type: 'enum', values: ['本地', '远程'], label: '运行位置' },
  url:         { type: 'string', label: '运行时URL' },
  publicUrl:   { type: 'string', label: '公网URL(双栖：本地进程 + 公网已部署)' },
...
function validate(mf) {
  ...
  if (mf.loc && ['本地', '远程'].indexOf(mf.loc) === -1) {
    errors.push('loc 值无效: "' + mf.loc + '"，合法值: 本地|远程');
  }
...
function crossValidateCategory(mf) {
  // 分类已确认：声明该 category 是刻意选择（对照过 CATEGORY_DEFINITIONS），跳过关键词门禁提示
  if (mf && mf.categoryAck === true) return [];
...
var CAT_SUGGEST = {
  '模型': 'service', 'Agent': 'service', '创作': 'service',
...
var LOC_OPTIONS = [
  { value: '本地', label: '本地', title: '本机进程/目录，本地运行' },
  { value: '远程', label: '远程', title: '外部 API/站点，云端运行' }
];
...
var TOOL_FIELDS = [
  ...
  { key: 'capability', input: 'f-cap', label: '能力（AI 路由用）', type: 'text', placeholder: '例：本地视觉模型理解图片', tooltip: '一句话能力，AI 判断这工具能不能干某活的信号。留空 = 保存时自动用「用途」前30字', section: '功能' },

  // ── 分类 ──
  { key: 'category', input: 'f-category', label: '功能分类', type: 'select', options: CATEGORY_OPTIONS, required: true, onchange: 'suggestType()', row: 'cls', section: '分类' },
  { key: 'categoryAck', input: 'f-category-ack', label: '分类已确认', type: 'checkbox', tooltip: '对照 CATEGORY_DEFINITIONS 确认分类正确后勾选，跳过关键词门禁提示。误报才勾，真存疑先去查分类定义', section: '分类' },
  { key: 'loc', input: 'f-loc', label: '运行位置', type: 'select', options: LOC_OPTIONS, required: true, onchange: 'suggestType()', row: 'cls', section: '分类' },
  { key: 'type', input: 'f-form', label: '类型', type: 'select', options: FORM_TYPES, required: true, onchange: 'applyFormType()', row: 'cls', section: '分类' },
  { key: 'dims', label: '派生展示（自动，从类型推导）', type: 'display', displayHtml: '<div class="auto-val" style="gap:16px;font-family:var(--font-body)"><span id="f-dims-acc"></span></div>', section: '分类' },
  ...
  { key: 'publicUrl', input: 'f-public-url', label: '公网 URL', type: 'text', placeholder: '例：https://forma.evopearl.com', tooltip: '双栖：本地有进程 + 公网已部署。填了卡片出「公网 ↗」', cg: 'service', formTypes: ['service'], section: '调用方式' },
  ...
  { key: 'return', input: 'f-ret', label: '返回', type: 'textarea', rows: 2, placeholder: '例：Web UI :3103/view/{slug}。留空 = 保存时按调用方式自动拼', tooltip: '留空则保存时按调用方式自动拼（调 API/URL/端口）', section: '何时调用' },
  { key: 'extraNotes', input: 'f-extra-note', label: '附加备注', type: 'textarea', rows: 2, placeholder: '例：【延迟】<1s【显存】8GB。运行备注，可留空', tooltip: '运行备注（延迟/显存/限速等），AI 从经验填，可留空。建议沿用【标签】值格式', section: '何时调用' },
