...
function registerStatic(app, ctx) {
  var { read, PROJECT_DIR, apiLog, listDirs, TOOLS_DIR, SKILLS_DIR, TIPS_DIR, PRINCIPLES_DIR } = ctx;
  ...
  app.get('/', function(req, res) {
    ...
    var cronTasks = (function(){
      try { var schedSt = JSON.parse(fs.readFileSync(path.join(os.homedir(), '.scheduler', 'data', 'scheduler-state.json'), 'utf8')); return Object.keys(schedSt.tasks || {}).length; } catch(_) { return 0; }
    })();
    var snap = JSON.stringify({
      ...
      assets: { tools: assetToolCount, skills: assetSkillCount, tips: assetTipCount, principles: assetPrincipleCount, api: apiEndpoints, cron: cronTasks, registry: 1 }
    });
