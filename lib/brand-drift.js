// lib/brand-drift.js — 品牌漂移审计：web/_style.css :root 品牌承载值必须 ⊆ vivi brand-dna.md 值集
...
function auditBrand(cssPaths) {
  var cssFiles = cssPaths || [
    path.join(__dirname, '..', 'web', '_style.css'),
  ...
  try {
    ...
    return { ok: false, total: cssFiles.length * 6, errors: 1, issues: ['品牌契约不可读: ' + BRAND_DNA + ' (' + e.message + ') — 契约搬走/改名须同步本模块路径'] };
  }
  ...
  cssFiles.forEach(function(css) {
    ...
    if (!root) { errors.push(css + ' 无 :root 块'); return; }
    var v = {};
    ...
    [['--green', '绿'], ['--green-deep', '深绿'], ['--yellow', '黄'], ['--green-soft', '淡黄绿'], ['--ink', '墨']].forEach(function(p) {
      ...
      if (!val) { errors.push(where + ' :root 缺品牌键 ' + key); return; }
      var hex = val.toUpperCase();
      if (!dnaHexes.has(hex)) errors.push(where + ' ' + label + ' ' + key + '=' + hex + ' 不在 brand-dna.md 色值集内 — 品牌漂移');
    });

    var fontHead = (v['--font-head'] || '').split(',')[0].replace(/['"]/g, '').trim();
    if (!fontHead) errors.push(where + ' :root 缺 --font-head');
    else if (!dnaTitleFont || fontHead !== dnaTitleFont) errors.push(where + ' 标题字体首族=' + fontHead + ' ≠ 契约=' + dnaTitleFont + ' — 字体漂移');
  });

  if (!dnaTitleFont) errors.push('brand-dna.md 未找到「标题栈 \'...\'」声明 — 契约格式变更须同步本模块解析');

  return { ok: errors.length === 0, total: cssFiles.length * 6, errors: errors.length, issues: errors };
}
