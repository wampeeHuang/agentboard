...
// HTTP 健康探活：端口有 HTTP 响应 → 视为已有服务在跑（docker 容器等外部服务）
function isPortResponding(port) {
  if (!port) return false;
  try {
    var out = child_process.execSync('curl -s -o nul -w "%{http_code}" --max-time 2 http://127.0.0.1:' + port, { timeout: 4000, encoding: 'utf8', shell: true, windowsHide: true });
    var code = out.trim();
...
// 共享运行时进程名：无端口工具用进程名判定运行态时跳过，避免任一 node/python/ffmpeg 进程都误报为工具在跑
var GENERIC_RUNTIME_NAMES = [
  'node.exe', 'npm.exe', 'npx.exe', 'yarn.exe', 'pnpm.exe',
  'python.exe', 'pythonw.exe', 'py.exe', 'pip.exe',
  'cmd.exe', 'powershell.exe', 'pwsh.exe', 'wt.exe', 'conhost.exe',
  'git.exe', 'bash.exe', 'sh.exe',
  'ffmpeg.exe', 'ffprobe.exe', 'docker.exe', 'curl.exe', 'wget.exe'
];
    ...
    names.forEach(function (name) {
      ...
      if (portsRunning === true) {
        ...
        if (pidAlive) {
          ...
          if (procAlive === false) {
            // 进程名兜底失败（docker CLI 不常驻等）→ HTTP 兜底：端口有响应视为外部服务已运行
            if (isPortResponding(primaryPort)) { running = true; runEvidence = '端口响应'; }
            else { running = false; }
          }
      ...
      // 无端口工具（桌面 GUI/CLI）→ 进程名兜底。只信任具体应用 exe，跳过共享运行时避免误报
      if (portsRunning === null && !pidAlive) {
        var nlProcName = getProcessName(mf);
        if (nlProcName && GENERIC_RUNTIME_NAMES.indexOf(nlProcName.toLowerCase()) === -1 && isProcessRunning(nlProcName) === true) {
          running = true;
          runEvidence = '进程名';
        }
      }
      ...
      var validation = schemaLoader.get().validate(mf);
      ...
      tools.push({
        ...
        icon: mf.icon || '', version: mf.version || '', category: mf.category, loc: mf.loc || '',
        ...
        apiBase: mf.apiBase, apiKeyName: mf.apiKeyName || '', models: mf.models || [],
        type: mf.type || 'service', trigger: mf.trigger || '',
        ...
        categoryAck: mf.categoryAck === true,
...
function _supervisorRestart(id, port, timeoutMs) {
  ...
  // 无端口工具（桌面 GUI/CLI）没有 TCP 端点，Supervisor 只管理端口服务（loadManifests 按 port 过滤）
  // 之前这里无脑回 delegated:true + ok:true = 假成功：Supervisor 收到 POST 但根本没拉进程
  _scanCache = null;
  _portsCache = null;
  return { delegated: false };
}
...
function startTool(id, dirs) {
  ...
  if (myPorts.length > 0) {
    ...
    if (orphaned.length > 0 && !selfPid) {
      // 端口被占但台账无进程（docker 容器等外部服务）：全部端口 HTTP 有响应 → 视为已在运行
      var responsive = orphaned.length === myPorts.length && orphaned.every(function (p) { return isPortResponding(p); });
      if (responsive) {
        _scanCache = null;
        _portsCache = null;
        opslog.info('tool-start', 'already running (external, port responsive): ' + id, { tool: id, ports: orphaned });
        return { ok: true, status: 'running', port: orphaned[0], note: '已在运行（端口有响应，外部服务如 docker 容器），直接使用' };
      }
...
var BASE_FIELDS = ['description', 'icon', 'version', 'category', 'loc', 'order', 'port', 'ports', 'projectPath', 'url', 'startCommand', 'stopCommand', 'publicUrl', 'owner', 'apiBase', 'type', 'trigger', 'agent_notes', 'capability', 'dashboard', 'disabled', 'runtime', 'children'];
...
function createTool(body, dirs) {
  ...
  // conflicts 为必填槽位：面板留空时 body 不含此键，此处落空数组（对齐 schema "无冲突写 []"）
  if (mf.conflicts === undefined) mf.conflicts = [];

  var portErr = checkPortUnique(mf.port, body.id, dirs);
  if (portErr) return { ok: false, error: portErr };

  var valid = schemaLoader.get().validate(mf);
