      ...
      if (portsRunning === true) {
        ...
        if (pidAlive) {
          ...
          if (portPid !== null && portPid !== pidAlive) {
            opslog.info('tool-stranger-port', name + ' port:' + primaryPort + ' occupied by PID ' + portPid + ' ≠ stored PID ' + pidAlive + ', killing stranger', { tool: name, port: primaryPort, strangerPid: portPid, storedPid: pidAlive });
            try { child_process.execSync('taskkill /PID ' + portPid + ' /T /F', { timeout: 5000, shell: true, windowsHide: true }); } catch (_) {}
            running = false;
          } else {
            running = true;
          }
...
function startTool(id, dirs) {
  ...
  try {
    ...
    var child;
    if (PLATFORM === 'win32') {
      child = child_process.spawn('cmd', ['/c', command], { cwd: cwd, detached: true, stdio: ['ignore', 'ignore', 'pipe'], env: spawnEnv });
    } else {
      child = child_process.spawn(command, { cwd: cwd, detached: true, stdio: ['ignore', 'ignore', 'pipe'], shell: true, env: spawnEnv });
    }
...
var BASE_FIELDS = ['description', 'icon', 'version', 'category', 'order', 'port', 'ports', 'projectPath', 'url', 'startCommand', 'stopCommand', 'publicUrl', 'owner', 'apiBase', 'type', 'trigger', 'agent_notes', 'capability', 'dashboard', 'disabled', 'runtime'];
