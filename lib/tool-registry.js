...
function getListeningPorts() {
  ...
  try {
    ...
    opslog.error('netstat-failed', 'netstat failed', { error: e.message });
  }
  _portsCache = ports;
  _portsCacheTime = now;
  return ports;
...
function getProcessName(mf) {
  ...
  if (mf.startCommand) {
    var words = mf.startCommand.trim().split(/\s+/);
    var first = words[0];
    if (/^(node(\.exe)?|cmd(\.exe)?)$/i.test(first) && words.length > 1) {
      first = words[1];
    }
    if (/\.exe$/i.test(first)) return first;
    if (/^[a-z][a-z0-9_-]+$/i.test(first)) return first + '.exe';
  }
  return null;
}
...
function getRunningProcesses() {
  var now = Date.now();
  if (_procsCache && (now - _procsCacheTime) < CACHE_TTL) return _procsCache;
  var procs = new Set();
...
function startTool(id, dirs) {
  ...
  try {
    ...
    // 轮询端口确认启动（最多 5 秒）
    if (mf.port) {
      var hlthDeadline = Date.now() + 5000;
      var portReady = false;
      ...
      if (!portReady) {
        opslog.error('tool-start-port-timeout', 'port ' + mf.port + ' not ready after 5s', { tool: id, port: mf.port });
        return { ok: false, error: 'Tool spawned but port ' + mf.port + ' did not respond within 5s. The process may have failed to start.' };
      }
    }
